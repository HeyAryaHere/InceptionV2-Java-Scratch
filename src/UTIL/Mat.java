...
public class Mat {
    ...
    public static float[] mm_add1x1(float[] m1, float[] m2) {
    ...
    public static float[][][] mm_add5x5(float[][][] m1, float[][][] m2) {
        float[][][] result = new float[m1.length][m1[0].length][m1[0][0].length];
        for (int i = 0; i < m1.length; i++) {
            for(int j = 0; j < m1[0].length; j++){
    ...
    public static float[] m_scale1x1(float[] mat, float scale) {
        float[] scl = new float[mat.length];
        for (int i = 0; i < mat.length; i++) {
                scl[i] = (float) mat[i] * scale;
        }
        return scl;
    }

    public static float[][][] m_scale5x5(float[][][] mat, float scale) {
        int depth = mat.length;
        int rows = mat[0].length;
        int cols = mat[0][0].length;
        float[][][] scaled = new float[depth][rows][cols];

        for (int d = 0; d < depth; d++) {
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    scaled[d][i][j] = (float) mat[d][i][j] * scale;
                }
            }
        }
     ...
     */
    public static float[][] m_random3x3(int h, int w) {
    ...
    public static float[] m_random1x1(int h) {
    ...
    public static float[] m_sub1x1(float[][] mat, int r_s, int r_e) {
        float[] sub = new float[r_e - r_s + 1];
        for (int i = 0; i < sub.length; i++) {
                sub[i] = mat[r_s + i][r_e];
        }
        return sub;
    ...
    public static float[][][] m_sub5x5(float[][][] mat, int d_s, int d_e, int r_s, int r_e, int c_s, int c_e) {
        int subDepth = d_e - d_s + 1;
        int subRows = r_e - r_s + 1;
        int subcol = c_e - c_s + 1;
        float[][][] sub = new float[subDepth][subRows][subcol];

        for (int d = 0; d < subDepth; d++) {
            for (int i = 0; i < subRows; i++) {
                for (int j = 0; j < subcol; j++)
                    sub[d][i][j] = mat[d_s + d][r_s + i][c_s + j];
            }
        }
        return sub;
    }
